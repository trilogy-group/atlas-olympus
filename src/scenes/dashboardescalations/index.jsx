...
import { performGetEscalations, getProductRealName, validateUser } from "../../data/fetchData";
...
function pickInitialWeek() {
  try {
    const raw = localStorage.getItem("escalations-history-data");
    if (raw) {
...
const DashboardEscalations = ({ bu_subset = "All", vpName = "All", productChosen = "All" }) => {
  // FIX: Normalize empty string to "All"
  ...
  useEffect(() => {
    ...
    const fetchDataAsync = async () => {
      try {
        let data = await performGetEscalations(globals);
        // console.log("🔍 [DASHBOARD] RAW DATA FROM performGetEscalations:", data);
    ...
    <Box m={isPortraitMobile ? "10px" : "20px"}>
      <Box display="flex" justifyContent="space-between" alignItems="center" flexDirection="column">
        <Box width="100%" display="flex" justifyContent="space-between" alignItems="center">
          <Header 
            title={`Central Support: Escalations through the weeks ( ${vpName === "All" ? "All VPs" : vpName} )`} 
...
export default DashboardEscalations;
