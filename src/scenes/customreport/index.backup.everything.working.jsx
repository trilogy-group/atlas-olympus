...
/*-------------- STATES ----------------------------------------------------------------------------*/
  ...
  const [prices, setPrices] = useState(null); // { GPT_4o: {in: 0.0000025, out: 0.00001}, ... }
...
/*------------------------------------------------------------------------------------------------*/
  ...
  const defaultFields = [
    ...
    "Tickets table - closedByMerge", "Tickets table - closedDueToMigration", "Tickets table - commentOther",
     ...
     "Tickets table - createdAt", "Tickets table - detailedStatus", "Tickets table - discardedAt", "Tickets table - discardedBy", 
     ...
     "Metrics table - l1FcrNew", "Metrics table - l2Fcr", "Metrics table - l2FcrNew", "Metrics table - npsComment", 
  ...
  useEffect(() => {
    if (baseInputTokens != null && baseOutputTokens != null) {
        ...
        const cost = (baseInputTokens * inputCostPerToken) + (baseOutputTokens * outputCostPerToken);

        setCalculatedCost(cost.toFixed(2));
        setcalculatedInputTokens(baseInputTokens);
        setcalculatedOutputTokens(baseOutputTokens);
    }
  }, [selectedOption, baseInputTokens, baseOutputTokens]);
  ...
  const handleProcess = async () => {
    ...
    if (!proceed) {
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        console.log(`Fifth call payload: ${JSON.stringify(payload, null, 2)}`);
        ...
        try {
          ...
          setPostProcessLogs((prevLogs) => [
              ...prevLogs,
              `[${getCurrentDate()}] - ${data5.message ? `Total cost: $${((inputCostPerToken * +sumInput) + (outputCostPerToken * +sumOutput)).toFixed(2)}` : " "}`,
          ]);
      ...
      ///////////////////////////// SIXTH CALL: Delete the buffer
        ...
        await checkProgress(payload.rows_count, "delete", 100);
  ...
  const handleOptionChange = (event) => {
      ...
      switch (selected) {
          case "GPT_4o":
              inputCostPerToken = 0.0000025; // $2.50 / 1M input tokens
              outputCostPerToken = 0.00001;  // $10.00 / 1M output tokens
              break;
          case "GPT_4o_mini":
              inputCostPerToken = 0.00000015; // $0.15 / 1M input tokens
              outputCostPerToken = 0.0000006;  // $0.60 / 1M output tokens
              break;
          case "GPT_o3_mini":
            inputCostPerToken = 0.00000110; // $1.10 / 1M input tokens
            outputCostPerToken = 0.0000044;  // $4.40 / 1M output tokens
            break;
          default:
      ...
      if (baseInputTokens != null && baseOutputTokens != null) {
          const cost = (baseInputTokens * inputCostPerToken) + (baseOutputTokens * outputCostPerToken);
          setCalculatedCost(cost.toFixed(2)); // Redondear a 4 decimales
          setcalculatedInputTokens(baseInputTokens);
          setcalculatedOutputTokens(baseOutputTokens);
          // console.log(`Calculated cost: $${cost}`);
      } else {
          console.error("Base tokens not initialized.");
      }
  };
  ...
  const initializeTokens = (inputTokens, outputTokens) => {
    ...
    const factor = selectedOption === "GPT_4o" ? 1 : 0.5; 
    setcalculatedInputTokens((inputTokens * factor).toFixed(2));
    setcalculatedOutputTokens((outputTokens * factor).toFixed(2));
  };
