...
/*------------------------------------------------------------------------------------------------*/
  ...
  const sanitizeTicketInput = (rawValue = "") => {
    const withoutSpaces = rawValue.replace(/\s+/g, "");
    const segments = withoutSpaces.split(",");
    const sanitizedSegments = segments.map((segment) =>
      segment.replace(/\D/g, "").slice(0, 8)
    );

    const endsWithComma = withoutSpaces.endsWith(",");
    if (!endsWithComma) {
      while (
        sanitizedSegments.length > 1 &&
    ...
    const filteredSegments = sanitizedSegments.filter((segment) => segment.length > 0);
    let formatted = filteredSegments.join(", ");

    if (endsWithComma && filteredSegments.length > 0) {
      formatted = `${formatted}, `;
    }
  ...
  const extractTicketIds = (rawValue = "") =>
    sanitizeTicketInput(rawValue)
      .split(",")
      .map((segment) => segment.trim())
      .filter((segment) => segment.length >= 7 && segment.length <= 8);

  const handleTicketIdsInput = (rawValue) => {
    const sanitized = sanitizeTicketInput(rawValue);
    setTicketIds(sanitized);
  };
  ...
  const handleAddField = () => {
    setFields((prevFields) => {
      const nextId =
        prevFields.length === 0
          ? 1
          : Math.max(...prevFields.map((field) => field.id || 0)) + 1;
  ...
  useEffect(() => {
    if (isProductMode) {
      const ready = Boolean(fromDate && toDate && selectedProducts.length > 0);
      setIsSecondBoxVisible(ready);
      return;
    }

    const hasTicketIds = ticketIds
      .split(",")
      .map((id) => id.trim())
      .filter(Boolean).length > 0;

    setIsSecondBoxVisible(hasTicketIds);
  }, [isProductMode, fromDate, toDate, selectedProducts, ticketIds]);
  ...
  const handleCreate = async () => {
    ...
    const baseFields = fields.slice(0, 2);
    const optionalFields = fields.slice(2);
    const updatedOptionalFields = optionalFields.map((field) => {
      const errors = {};
    ...
    const validatedFields = [...baseFields, ...updatedOptionalFields];
    ...
    const ticketIdsList = isProductMode ? [] : extractTicketIds(ticketIds);
    ...
    if (isProductMode) {
      ...
      if (ticketIdsList.length === 0) {
        popUp("Please enter at least one valid ticket ID (7-8 digits).");
        setLoading(false);
        return;
      }
    }
    ...
    const productIds = isProductMode
      ? selectedProducts
          .map((prod) => {
    ...
    const result = {
      ...
      columns: updatedOptionalFields.length,
      columnNames: updatedOptionalFields.map((field) => [
        field.name || "",
        field.customName || "",
        field.options || "",
        field.description || "",
    ...
    console.log(`[CUSTOM REPORT] Payload enviado al Lambda:`, JSON.stringify(payload, null, 2));
      ...
      const hasCustomFields = updatedOptionalFields.some(field => 
        field.name === "Custom" &&
    ...
    if (data3.message && data3.message.total_input_tokens != null) {
      ...
      const calibrationSnapshot = data3.message.calibration_snapshot || {};
      const snapshotInputMultiplier = parseFloat(calibrationSnapshot.input_multiplier ?? 1);
      const snapshotOutputMultiplier = parseFloat(calibrationSnapshot.output_multiplier ?? 1);
      const snapshotBaseInput = parseFloat(calibrationSnapshot.base_input_tokens ?? totalInputTokens);
      const snapshotBaseOutput = parseFloat(calibrationSnapshot.base_output_tokens ?? totalOutputTokens);

      setTokenCalibration({
        inputMultiplier: Number.isFinite(snapshotInputMultiplier) ? snapshotInputMultiplier : 1,
        outputMultiplier: Number.isFinite(snapshotOutputMultiplier) ? snapshotOutputMultiplier : 1,
        baseInputTokens: Number.isFinite(snapshotBaseInput) ? snapshotBaseInput : null,
        baseOutputTokens: Number.isFinite(snapshotBaseOutput) ? snapshotBaseOutput : null,
      });
  ...
  const handleProcess = async () => {
    ...
    const currentTicketIds = isProductMode ? [] : extractTicketIds(ticketIds);
    if (!isProductMode && currentTicketIds.length === 0) {
      popUp("Please enter at least one valid ticket ID (7-8 digits).");
    ...
    payload.ticketIDs = currentTicketIds;
    ...
    if (!proceed) {
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        const customColumnsCount = fields.filter((field) => field.id > 2).length;
        const estimatedInputForPayload = Number(
          calculatedInputTokens ?? baseInputTokens ?? 0
        );
        const estimatedOutputForPayload = Number(
          calculatedOutputTokens ?? baseOutputTokens ?? 0
        );
        const calibrationSnapshotForPayload = {
            input_multiplier: Number.isFinite(tokenCalibration.inputMultiplier) ? tokenCalibration.inputMultiplier : 1,
            output_multiplier: Number.isFinite(tokenCalibration.outputMultiplier) ? tokenCalibration.outputMultiplier : 1,
            base_input_tokens: Number.isFinite(tokenCalibration.baseInputTokens) ? tokenCalibration.baseInputTokens : null,
            base_output_tokens: Number.isFinite(tokenCalibration.baseOutputTokens) ? tokenCalibration.baseOutputTokens : null,
        };
        ...
        let payload2 = {
            ...
            estimated_input_tokens: estimatedInputForPayload,
            estimated_output_tokens: estimatedOutputForPayload,
            calibration_snapshot: calibrationSnapshotForPayload,
            model: selectedOption,
            columns_count: customColumnsCount,
  ...
  const handleSaveConfig = (event) => {
    const ticketIdsList = extractTicketIds(ticketIds);
    const hasColumns = (fields?.length ?? 0) > 0;

    if (isProductMode) {
      if (!fromDate || !toDate || selectedProducts.length === 0 || !hasColumns) {
        popUp("Please complete dates, products and columns before saving.");
        return;
      }
    } else {
      if (ticketIdsList.length === 0 || !hasColumns) {
        popUp("Please enter at least one valid ticket ID and have columns before saving.");
        return;
      }
    }
  ...
  const handleConfirmSaveConfig = async () => {
    ...
    const ticketIdsList = extractTicketIds(ticketIds);
    const jsonPayload = isProductMode
      ? {
          mode: 'PM',
          selectedProducts,
  ...
  const handleSelectConfig = (config) => {
    const parsedConfig = JSON.parse(config[2]); // Parse configuration

    const mode = parsedConfig.mode || 'PM';
    const ticketsFromConfig = parsedConfig.ticketIds || '';

    if (mode === 'TM') {
      setIsProductMode(false);
      productMode = false;
      setTicketIds(ticketsFromConfig);
      setSelectedProducts([]);
      setFromDate(null);
      setToDate(null);
    } else {
      ...
      productMode = true;
      setTicketIds('');
      setFromDate(parsedConfig.fromDate ? dayjs(parsedConfig.fromDate) : null);
      setToDate(parsedConfig.toDate ? dayjs(parsedConfig.toDate) : null);
      setSelectedProducts(parsedConfig.selectedProducts || []);
    }
    const normalizedFields =
      parsedConfig.fields && parsedConfig.fields.length > 0
        ? (() => {
            const usedIds = new Set();
            return parsedConfig.fields.map((field, idx) => {
              let nextId = typeof field.id === "number" ? field.id : idx + 1;
    ...
    setIsConfigPanelVisible(false); // hide panel
  };
  ...
  const toggleMode = () => {
    ...
    // Clear only mode-specific inputs; keep columns and logs
    if (next) {
      // Switched to Product Mode: drop ticket IDs only
      setTicketIds("");
    } else {
      // Switched to Ticket Mode: drop product/date filters only
      setSelectedProducts([]);
      setFromDate(null);
      setToDate(null);
    }
              ...
              {isProductMode ? (
                ...
                <Box sx={{ width: "100%" }}>
                  <TextField
                    ...
                    value={ticketIds}
                    onChange={(e) => handleTicketIdsInput(e.target.value)}
          ...
          fields.map((field, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Typography variant="h6" sx={{ mb: 2 }}>
                    {`Column ${columnIndex} ${ isMandatory ? " (This field is mandatory) " : ""}`}
                  </Typography>
